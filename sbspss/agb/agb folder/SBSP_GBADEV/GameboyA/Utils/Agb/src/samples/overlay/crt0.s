...
@--------------------------------------------------------------------
@-          Interrupt Branch Process (Draw from Table) 32Bit  25-62c -
@--------------------------------------------------------------------
    .EXTERN     IntrTableBuf
    .GLOBAL     intr_main
    .ALIGN
...
intr_main:
        ...
        mov     r2, #0                       @ Check IE/IF
        ands    r0, r1, #V_BLANK_INTR_FLAG   @ V Blank interrupt
        strneh  r0, [r12, #INTR_CHECK_BUF - WRAM_END] @ Set interrupt check
        bne     jump_intr
        add     r2, r2, #4
        ands    r0, r1, #H_BLANK_INTR_FLAG   @ H Blank interrupt
        bne     jump_intr
        add     r2, r2, #4
        ands    r0, r1, #V_COUNT_INTR_FLAG   @ V Balnk interrupt
        bne     jump_intr
        add     r2, r2, #4
