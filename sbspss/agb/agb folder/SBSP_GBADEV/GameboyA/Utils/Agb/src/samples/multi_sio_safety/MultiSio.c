...
void MultiSioInit(void)
{
	...
	*(vu16 *)REG_SIOCNT |= SIO_IF_ENABLE | MULTI_SIO_BAUD_RATE;
	...
	for (i=0; i<4; i++) {                               // Sets receive buffer pointer
		...
		Ms.RecvBakBufp[i]     = (u16 *)&Ms.RecvBuf[i][3];
	}
...
void MultiSioSendDataSet(void *Sendp)
{
	...
	CpuCopy(       (u8 *)&Ms.NextSendBufp[2] + MULTI_SIO_BLOCK_SIZE, // Moves previously sent data
				   (u8 *)&Ms.NextSendBufp[2],  MULTI_SIO_BLOCK_SIZE*2, 32);
	CpuCopy(Sendp, (u8 *)&Ms.NextSendBufp[2] + MULTI_SIO_BLOCK_SIZE, // Sets send data
											   MULTI_SIO_BLOCK_SIZE,   32);

	for (i=0; i<sizeof(MultiSioPacket)/2 - 2; i++)      // Calculate send data check sum
...
u32 MultiSioRecvDataCheck(void *Recvp)
{
	...
	*(vu16 *)REG_IME = 0;                               // Disables interrupt (about 80 clocks)
	...
	for (i=0; i<4; i++) {
		BufpTmp = Ms.RecvCheckBufp[i];                  // Update received data check buffer
	...
	*(vu16 *)REG_IME = 1;                               // Enables interrupt
	...
	for (i=0; i<4; i++) {
		...
		CheckSum = 0;                                   // Calculate received data check sum
		for (ii=0; ii<sizeof(MultiSioPacket)/2 - 2; ii++)
			CheckSum +=  Ms.RecvCheckBufp[i][ii];
		if ((s16 )CheckSum == -1) {
			BufpTmp = Ms.RecvBakBufp[i];                // Update received data backup
			Ms.RecvBakBufp[i] = Ms.RecvCheckBufp[i];
			Ms.RecvCheckBufp[i] = BufpTmp;
			Ms.RecvBlockOffset[i] = 0;
		} else
			Ms.RecvBlockOffset[i] = MULTI_SIO_BLOCK_SIZE;

		CounterDiff = Ms.RecvFrameCounter[i][0] - Ms.RecvFrameCounter[i][1];

		if (CounterDiff == 1 && SyncRecvFlagBak[i])    	// When received successfully (or failed to receive 1 frame)
			if ((s16 )CheckSum == -1 || Ms.RecvCheckSumBak[i] == -1) {
				CpuCopy(&((u8 *)Ms.RecvBakBufp[i])[4 + Ms.RecvBlockOffset[i]],
						&((u8 *)Recvp)[i*MULTI_SIO_BLOCK_SIZE], MULTI_SIO_BLOCK_SIZE, 32);
				Ms.RecvSuccessFlags |= 1 << i;
			}

		Ms.RecvCheckSumBak[i] = CheckSum;
	}

    Ms.ConnectedFlags |= Ms.RecvSuccessFlags;           // Sets connection completion flag

    if (Ms.ConnectedFlags ^ Ms.RecvSuccessFlags)        // Update synchronous adjustment counter
...
void MultiSioIntr(void)
{
	...
	*(u64 *)RecvTmp = *(u64 *)REG_SIOMLT_RECV;
	...
	if (Ms.SendBufCounter != -1) {                      // Sets send data
		...
		for (ii=0; ii<1; ii++)      ;                   // Send timing adjustment wait
	} else {
		((SioMultiCnt *)REG_SIOCNT)->Data = MULTI_SIO_SYNC_DATA;
		                                                // Sets synchronous data

		BufpTmp = Ms.CurrentSendBufp;                   // Send buffer change
	...
	// Received data process (included in 350 clocks/wait period)
	...
	for (i=0; i<4; i++) {
		if (RecvTmp[i] == MULTI_SIO_SYNC_DATA
		 && Ms.RecvBufCounter[i] > (s32 )(sizeof(MultiSioPacket)/2 - 3)) {
			...
			Ms.SyncRecvFlag[i] = 1;
			for (ii=0; ii<5; ii++)      ;               // Send timing adjustment wait
		} else {
			...
			else	for (ii=0; ii<2; ii++)      ;       // Send timing adjustment wait
		}
		if (Ms.RecvBufCounter[i] < (s32 )(sizeof(MultiSioPacket)/2 - 1))	Ms.RecvBufCounter[i]++;
	}


	// Master restarts sending data

	if (Ms.Type == SIO_MULTI_PARENT) {
		for (i=0; i<MULTI_SIO_SEND_WAIT_COUNT; i++)   ; // Send wait adjustment
