...
jump_intr:
        ...
        ldr     r1, =TIMER0_INTR_FLAG               @ Set IE  <- Multiple Interrupt Choice
        strh    r1, [r3]
        mrs     r3, cpsr                            @ Multiple Interrupt Permission & Switch to System Mode
        bic     r3, r3, #PSR_CPU_MODE_MASK | PSR_IRQ_DISABLE | PSR_FIQ_DISABLE
        orr     r3, r3, #PSR_SYS_MODE
        msr     cpsr, r3
@---------------------------------------------------------------------System Mode
        ldr     r1, =IntrTable                      @ ump to User IRQ Process
        add     r1, r1, r2
        ldr     r0, [r1]
