...
@--------------------------------------------------------------------
@- Interrupt Branch Process (Multiple Interrupt)(Table Lookup)43-81c-
@--------------------------------------------------------------------
    .EXTERN     IntrTable
    .GLOBAL     intr_main
    .ALIGN
...
intr_main:
        ...
        bne     jump_intr
        add     r2, r2, #4
        ands    r0, r1, #CASSETTE_INTR_FLAG  @ Cartrige interrupt
loop:   bne     loop
jump_intr:
        strh    r0, [r3, #2]              @ IF clear        22c
                                          @ IE set <- Multiple interrupt choice
        ldr     r1, =CASSETTE_INTR_FLAG | TIMER0_INTR_FLAG
        strh    r1, [r3]
        mrs     r3, cpsr     @ Multiple Int. Permission&Switch to System Mode
...
intr_return:
        ...
        mrs     r3, cpsr     @ Multiple Int. Not Permitted & Switch to IRQ Mode
        bic     r3, r3, #PSR_CPU_MODE_MASK | PSR_IRQ_DISABLE | PSR_FIQ_DISABLE
