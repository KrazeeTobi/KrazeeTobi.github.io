...
void MultiSioInit(void)
{
	...
	*(vu16 *)REG_SIOCNT |= SIO_IF_ENABLE | MULTI_SIO_BAUD_RATE;
...
#ifdef MULTI_SIO_DI_FUNC_FAST                           // Copies functions
	...
	for (i=0; i<4; i++) {                               // Sets received buffer pointer
...
u32 MultiSioRecvDataCheck(void *Recvp)
{
...
#ifdef MULTI_SIO_DI_FUNC_FAST                           // Update received data check buffer
	...
	*(vu16 *)REG_IME = 1;                               // Enables interrupt
	...
	for (i=0; i<4; i++) {
		...
		if (CounterDiff == 1 && SyncRecvFlagBak[i])    	// When receiving succeeded
			if ((s16 )CheckSum == -1) {
