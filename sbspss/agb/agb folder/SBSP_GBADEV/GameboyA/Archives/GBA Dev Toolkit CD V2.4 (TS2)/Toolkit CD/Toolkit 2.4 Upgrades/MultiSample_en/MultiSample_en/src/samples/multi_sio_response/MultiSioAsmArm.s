;********************************************************************
;*          MultiSioAsmArm.s                                        *
;*            Multi-Play Communication Library (ARMASM)                *
;*            (Common to Resonse Emphasized/Communication Error Countermeasure versions)      *
;*                                                                  *
...
MultiSioIntr
        ...
        ldr     r11, =Ms                                ; r11: &Ms
        ldr     r9,  =MULTI_SIO_SYNC_DATA               ; r9:  MULTI_SIO_SYNC_DATA
...
3
        ...
        ; Pushes received data and register value to stack

        stmfd   sp!, {r0-r1, r5-r6}



        ; Received data process (included in 160 clocks/wait period)
...
4
        ...
        ;     for (ii=0; ii<2; ii++)      ;                   Send timing adjustment wait
...
9
        ...
        ;     for (i=0; i<MULTI_SIO_SEND_WAIT_COUNT; i++)   ; Adjusts send wait

    IF  MULTI_SIO_SEND_WAIT_COUNT > 0
        mov     r0, #MULTI_SIO_SEND_WAIT_COUNT - 1
10      subs    r0, r0, #1
        bge     %b10
    ENDIF
