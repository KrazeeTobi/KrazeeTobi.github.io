...
MULTI_SIO_INTR_DELAY_MAX        *   2048    ; Commmunication interrupt acceptable delay clock number 
...
;* Sets serial communication mode to multi-play mode.
...
;* Calls MultiSioRecvDataCheck(), checks if data is received normally, and copies the received data to Recvp.
;
...
;If Master, restart sending. 
;
;* Program it so that it is called during communication interrupt.
;* Adjust set values so that 1 packet (other than OverRunCatch[]) can certainly be transferred in 1 frame.
...
;* Checks if data is received normally, and if so, copies the received data to uyser recieve buffer.
;
;* Called from MultiSioMain().
;* It is not necessary to call directly.
;
