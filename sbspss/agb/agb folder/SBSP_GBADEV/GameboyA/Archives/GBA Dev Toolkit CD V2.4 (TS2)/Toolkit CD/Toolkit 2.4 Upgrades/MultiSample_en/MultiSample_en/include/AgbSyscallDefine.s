...
@ * Reset the register and RAM that were specified by ResetFlags and return to address 0x8000000.
@ * The area between 0x3007e00 and 0x3007fff of CPU core register and CPU internal RAM is forced to be cleared.
@
@ * Arguments:
@    r0:                  Specify the register and RAM that are to be reset (See AgbDefine.s for reference)
...
@ * When applicable interrupt is enabled (set in IE), return with the generation of the conditions 
@  for an interrupt request from a key, a game pak or SIO.
...
@ * Set a flag that corresponds to INTR_CHECK_BUF(0x3007ff8) by interrupt process.
@ * Overhead that calls for system call can be reduced by using multiple interrupts simultaneously when compared
@  with the case where Halt() is called back repeatedly.
@
@ * Arguments:
@    r0:                 Specify whether or not to clear when applicable flag has already been set.
@    r1:                 Specify an interrupt that waits. (See AgbDefine.s for reference)
...
@ * Set a flag that corresponds to INTR_CHECK_BUF(0x3007ff8) in interrupt process.
@ * Overhead that calls for system call can be reduced by using multiple interrupts simultaneously when compared 
...
@ * In order to make it accurate, shift argumentX by the multiple of 2 to the left, and shift return value to the right
...
@ * For return value, return -pi/2ƒƒÆƒpi/2 within the range of 0xc000~0x4000.
@ * However, the range ƒÆƒ-pi/4,pi/4ƒƒÆ has a problem with accuracy.
@
@ * Arguments:
@    r0:
@        Sign: 1
...
@ * For return value, return 0…ƒÆƒ2pi within the range of 0~0xffff.
@
@ * Arguments:
@    r0, r1:
@        Sign: 1
...
@ * Clear or copy RAM by using parameter that is compatible with DmaSet macro.
@ * RAM is forcefully accessed at every 4 byte boundary by 32bit transfer, and it is to be accessed at every 2 byte boundary by 16bit transfer.
@ 
@
@ * Arguments:
@    r0:                 Source address
@    r1:                 Destination address
@    r2:                 Only DMA_SRC_FIX/DMA_32BIT_BUS/DMA_COUNT_MASK are valid
...
@ * Clear or copy RAM in high speed by using parameter that is compatible with DmaSet macro.
@ * However, it is a 32bit transfer per 32 bytes (8 words).
@ * Even if an argument other than 4byte boundary is given, it is accessed forcefully at every 4 byte boundary.
@
@ * Arguments:
@    r0:                 Source address
@    r1:                 Destination address
...
@    s32 SrcCenterX            Source data center coordinate (8bits to the right of decimal point)
@    s32 SrcCenterY
@    s16 DispCenterX           Display center coordinate
@    s16 DispCenterY
@    s16 RatioX                Scaling ratio (8bits to the right of decimal point)
@    s16 RatioY
@    u16 Theta                 Rotation angle (8bits to the right of decimal point)
...
@    r1:                       Destination address
@    r2:                       Number of times calculated
@    r3:                       Offset byte size of parameter address (normally 2|8)
...
@  Open music player.
@  "ma" is a work area, "ta" is a work area by track, and "tn" is the number of track (maximum 16).
@  If "ma and, ta[tn]" are prepared separately, multiple players can be opened, which enables playing multiple
@  songs simultaneously.  For example, you can open one music player for BGM and four music players for SE.
@  
@  *Music player function gruop is the upper function of sound driver function group.
...
@  Play a song "so" by using the music player of the work area "ma".

@--------------------------------------------------------------------
@-                     Pause a song that is played                       -
@--------------------------------------------------------------------

    .EXTERN MusicPlayerStop

@  Pause the song that is currently played by the music player of the work area "ma".

@--------------------------------------------------------------------
@-                        Re-open the song that is paused.                            -
@--------------------------------------------------------------------

    .EXTERN MusicPlayerContinue

@  Re-open the song that is paused in the music player of the work area "ma".
...
@  Fade out and stop teh song that is currently played in the work area "ma" by the speed "sp".
