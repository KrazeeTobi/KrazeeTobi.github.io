...
#define MULTI_SIO_BLOCK_SIZE        16      // Communication data block size
...
typedef struct {
	...
	u16 Data[2][MULTI_SIO_BLOCK_SIZE/2];    // Communication data
...
typedef struct {
    ...
    u8  RecvBlockOffset[4];                 // Received data block assignment

    s16 RecvCheckSumBak[4];                 // Previously received data check sum

    MultiSioPacket  SendBuf[2];             // Send buffer (double buffer)
    MultiSioPacket  RecvBuf[4][4];          // Receive buffer  (four-fold buffer) 
} MultiSioArea;
...
//* Calls MultiSioRecvDataCheck(), checks if data has been received normally, 
//  and copies received data to Recvp.
//
//* Set it so taht it is called at the closest timing within 1 frame.  
...
//* Adjust the set value so that 1 packet (other than OverRunCatch[]) can be certainly transferred in 1 frame.
...
//* Checks if data is received normally, and if so, copies received data to user receive buffer. 
//* If it is a failure of 1 frame, make a copy from the same data which was sent in the previous frame.
//  However, it cannot be recovered when frame counter area is destroyed.  
//
//* Called from MultiSioMain().
//* It is not necessary to call directly.  
//
