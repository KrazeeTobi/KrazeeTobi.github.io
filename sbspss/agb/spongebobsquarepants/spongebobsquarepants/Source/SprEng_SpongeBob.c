...
#ifdef MUSIC_ON
        m4aSongNumStart(SFX_SPONGEBOB_LAND_AFTER_FALL);//BGM Start
#endif		
		}
...
#ifdef MUSIC_ON
        m4aSongNumStart(SFX_SPONGEBOB_JUMP);//BGM Start
#endif		
			}
...
#ifdef MUSIC_ON
        m4aSongNumStart(SFX_SPONGEBOB_KARATE);//BGM Start
#endif
			}
		}
...
// Run.
	...
	case MODE_RUN:					// Sprite in this mode ?.
		...
		if(gPushScrollY>0) {gPushScrollY-=LOOKSPEED;}
                ...
                if(pAO->sp_aniframe==0)
                {
#ifdef MUSIC_ON
		        m4aSongNumStart(SFX_SPONGEBOB_WALK_1);//BGM Start
#endif	        
		}
                if(pAO->sp_aniframe==8)
                {
#ifdef MUSIC_ON
		        m4aSongNumStart(SFX_SPONGEBOB_WALK_2);//BGM Start
#endif	        
		}
...
#ifdef MUSIC_ON
        m4aSongNumStart(SFX_SPONGEBOB_BUTTBOUNCE);//BGM Start
#endif
		}
